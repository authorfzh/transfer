...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
    ...
    var t = layui.$,
        ...
        z = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# if(d.data.toolbar){ }}", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '<div class="layui-table-init" style="background-color: #fff;">', '<i class="layui-icon layui-icon-loading layui-icon"></i>', "</div>", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', _(), "</div>", '<div class="layui-table-body layui-table-main">', E, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', _({
            ...
            fixed: "right"
        }), '<div class="layui-table-mend"></div>', "</div>", '<div class="layui-table-body">', E, "</div>", "</div>", "{{# }; }}", "</div>", "{{# if(d.data.totalRow){ }}", '<div class="layui-table-total">', '<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<tbody><tr><td><div class="layui-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>', "</table>", "</div>", "{{# } }}",'<div class="table_foot">','<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', "{{# if(d.data.self){ }}", '<div class="layui-table-tool-self"></div>', "{{# } }}","</div>",  "{{# if(d.data.page){ }}", '<div class="layui-table-page">', '<div id="layui-table-page{{d.index}}"></div>', "</div>", "{{# } }}",'</div>' ,"<style>", "{{# layui.each(d.data.cols, function(i1, item1){", "layui.each(item1, function(i2, item2){ }}", ".laytable-cell-{{d.index}}-{{i1}}-{{i2}}{ ", "{{# if(item2.width){ }}", "width: {{item2.width}}px;", "{{# } }}", " }", "{{# });", "}); }}", "</style>", "</div>"].join(""),
        H = t(window),
        R = t(document),
        F = function(e) {
            var i = this;
    ...
    F.prototype.config = {
     ...
     // 表格宽度自适应开始
        var cols_width_count = a.cols[0].length,cols_width_sum = 0 , elem_width = a.width ? a.width :a.elem.parent().width();
        for(var cols_width = 0 ;cols_width < a.cols[0].length;cols_width++){
            if(a.cols[0][cols_width].width!=undefined){
                cols_width_count--,cols_width_sum += Number (a.cols[0][cols_width].width);
            //	console.log("cols_width_sum"+cols_width_sum);
            }
        }
        var cols_width_fix = parseInt((elem_width - cols_width_sum-13) / cols_width_count);
        if(cols_width_fix > 0){
            for(cols_width = 0 ;cols_width < a.cols[0].length;cols_width++){
                if(!a.cols[0][cols_width].width){
                    a.cols[0][cols_width].width = cols_width_fix;
                }
            }
        }
        
        /* var othis = a.elem
        ,hasRender = othis.next('.layui-table-view' );
        
       if(a.height && /^full-\d+$/.test(a.height)){ //full-差距值
          that.fullHeightGap = a.height.split('-')[1];
          a.height = _WIN.height() - that.fullHeightGap;
        }*/
        // 表格宽度自适应结束
      
      //  console.log("elem_width"+elem_width+"\ncols_width_sum"+cols_width_sum+"\ncols_width_count"+cols_width_count);
                ...
                return !r && c.sortKey ? c.sort(c.sortKey.field, c.sortKey.sort, !0) : (layui.each(u, function(a, l) {
                    ...
                    0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function(n, r) {
                        ...
                        if (void 0 !== p && null !== p || (p = ""), !r.colGroup) {
                            ...
                            function() {
                                var e = [];
                                return r.hide && e.push(y), r.field || e.push("_layui-table-col-special"), e.join(" ")
      ...
      //  console.log(i);
        if (i.totalRow) {
            ...
            t.eachCols(function(e, t) {
                var n = t.field || e,
                    ...
                    function() {
                        var e = [];
                        return t.hide && e.push(y), t.field || e.push("_layui-table-col-special"), e.join(" ")
                    }() + '">', '<div class="layui-table-cell laytable-cell-' +
                    function() {
                        var e = i.index + "-" + t.key;
                    ...
                    function() {
                        ...
                        var e = t.totalRowText || "";
	                   	//判断计算统计
	                   if(t.totalRow=="true" && t.transform=="residenceTime"){
	                    		var shijiancha= parseFloat(a[n]).toFixed(2);
	                    		var days    = shijiancha / 60 / 60 / 24;
	                    		var daysRound   = Math.floor(days);
	                    		var hours    = (Math.floor(shijiancha/ 60 / 60)) % 24;
	                    		var hoursRound   = Math.floor(hours);
	                    		var minutes   = (Math.floor(shijiancha  /60)) % 60;
	                    		var minutesRound  = Math.floor(minutes);
	                    		var seconds   = (Math.floor(shijiancha )) % 60;
	                    		str= daysRound + " 天 " +hoursRound + " 小时 " +minutesRound + " 分钟 " + Math.floor(seconds) + " 秒";
	                    	}else{
	                    		str=t.totalRow ? parseFloat(a[n]).toFixed(2) || e : e
	                    	}
	                   	return str;
                    }(), "</div></td>"].join("");
                l.push(o)
        ...
        a.layTool.on("click", "*[lay-event]", function(e) {
            ...
            case "LAYTABLE_PRINT":
                ...
                v.append(a.layMain.find("table").html()), v.find("th.layui-table-patch").remove(), v.find(".tt-lay_layui-table-col-special").remove(), h.document.write(f + v.prop("outerHTML")), h.document.close(), h.print(), h.close()
            }
        ...
        }).on("mouseenter", ".layer-span", function() {
        
        	//获取span的父级 tr
        	 var a=$(this).parent().parent();
        	 //span的宽度
        	 var span_width=$(this).prop("offsetWidth");
        	 //tr的宽度
        	 var tr_width=a.prop("scrollWidth");
        	 //获取span的父级 div
         	 var div_width=$(this).parent().prop("offsetWidth");
        	// console.log("span_width:"+span_width+"-------------tr_width:"+tr_width+"-----------div_width:"+div_width)
        	 //判断长度是否溢出  span的宽度大于盒子div的宽度是为溢出
        	 if (span_width > div_width && a.prop("className")=="tt-lay"){
         		layer.tips("<span style='color:#333333;font-size:12px'>"+$(this).text()+"</span>", $(this), {
            		  tips: [1, '#ffffff'] //还可配置颜色
            		,time:0
            		/*,area: ['auto', 'auto']*/
            		});
        	 }	
        }).on("mouseleave", ".layer-span", function() {
        	layer.closeAll('tips');
        });
